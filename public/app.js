...
const labels={pending:'Not reviewed',ok:'OK for website',no:'No — leave off',fix:'Needs changes',applied:'Title applied'};
let data=null,group='held',page=0,selected=null,dirty=false,busy=false;
const pageSize=25;
function matches(item,d){return d && (d.fingerprint===item.fingerprint || item.compatibleFingerprints?.includes(d.fingerprint));}
function status(item){if(item.applied)return 'applied';const d=data.decisions[item.id];return matches(item,d)?d.decision:'pending';}
...
function renderGroups(){
  $('groups').innerHTML=Object.entries({held:'Website items to check',outside:'Other items not on website'}).map(([key,label])=>`<button class="tab ${group===key?'active':''}" data-group="${key}">${label} · ${data.items.filter((r)=>r.group===key&&status(r)==='pending').length.toLocaleString()}</button>`).join('');
  $('groups').querySelectorAll('[data-group]').forEach((button)=>button.onclick=()=>{if(!selectionAllowed())return;group=button.dataset.group;$('status').value='pending';applyFilters();document.body.classList.remove('detail-open');});
}
function filtered(){const q=$('search').value.trim().toLowerCase(),filter=$('status').value,brand=$('brand').value;
  return data.items.filter((r)=>['held','outside'].includes(r.group) && r.group===group && (filter==='all'||status(r)===filter) && (!brand||r.brand===brand)
    && (!q||[r.sku,r.name,r.brand,r.source?.name,...r.issues.map((i)=>i.issue),data.decisions[r.id]?.note].join(' ').toLowerCase().includes(q)));}
function renderList(){
  ...
  const remaining=data.items.filter((r)=>r.group===group&&status(r)==='pending').length;
  $('progress').textContent=`${remaining.toLocaleString()} left to check`;
  renderGroups();
  return rows;
}
...
function renderDetail(){
  ...
  if(!item){$('detail').innerHTML='<p class="empty">Choose an item from the list to review its issue and proposed fix.</p>';return;}
  if(item.applied){
...
async function save(decision){
  ...
  const before=filtered(),at=before.findIndex((r)=>r.id===selected);
  ...
  try{
    ...
    notice(`Saved ${item.sku||'item'}: ${labels[decision]}. Nothing was published.`);
    if(decision!=='pending'){
      const next=[...before.slice(at+1),...before.slice(0,at)].find((r)=>r.id!==item.id&&status(r)==='pending');
      selected=next?.id||filtered()[0]?.id||null;
    }
    const after=filtered(),newIndex=after.findIndex((r)=>r.id===selected);if(newIndex>=0)page=Math.floor(newIndex/pageSize);
...
async function load(){
  ...
  try{
    const response=await fetch('/api/review');if(response.status===401){window.location.assign('/signin-with-chatgpt?return_to=%2Freview');return;}if(!response.ok)throw new Error((await response.json()).error||'Could not load your online review.');
